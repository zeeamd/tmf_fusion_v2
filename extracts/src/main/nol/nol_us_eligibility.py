...
# local dependencies
from extracts.src.main.commonsql.nol_adaptor_data_pull import get_report_credit_sql, get_nol_us_eligibility_sql

spark = utils.spark_session("NOL Eligibility Integration")
...
def main(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm):
    """
    Describe NOL Eligibility US demos transformations and actions here.
    """
    # spark is an existing SparkSession

    # ETL_REPORT_CREDIT_MONTHLY_V
...
try:
    ...
    proj_factors_sdf_frmt = get_nol_us_eligibility_sql(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm)
    proj_factors_sdf = utils.build_dataframe(spark, lambda: get_nol_us_eligibility_sql(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm))
    ...
    utils.writeToLog("Unable to fetch required data for eligibility criteria from database with error: "  + str(e) )
    exit(1)

#proj_factors_sdf.write.csv("s3://useast1-nlsn-mdl-w-tam-totalmediafusion-dev/us_eligibility_201827.csv")
proj_factors_sdf.repartition(2).write.format("com.databricks.spark.csv").option("header", "true").option("timestampFormat", "yyyy-MM-dd HH:mm:ss").mode('overwrite').csv("s3://useast1-nlsn-mdl-w-tam-totalmediafusion-dev/us_eligibility_201802.csv")
...
if __name__ == "__main__":
    ...
    spark.stop()
    utils.writeToLog("NOL Eligibility Application Completed")
