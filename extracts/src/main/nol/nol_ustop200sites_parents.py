"""
File: nol_ustop200sites_parents.py
Author: KiranK
...
# local dependencies
from extracts.src.main.commonsql.nol_adaptor_data_pull import get_nol_us_top200sites_parents_sql, \
    get_report_credit_sql


spark = utils.spark_session("NOL US Top200 Sites Integration")
...
def main(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm, parvw_nm):
    """
...
try:
    utils.writeToLog("Running NOL USTop200 Sites Steps")
    ...
    rpt_parents_etl = get_nol_us_top200sites_parents_sql(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm, parvw_nm)

    rpt_parents_etl_sdf = utils.build_dataframe(spark, lambda: get_nol_us_top200sites_parents_sql(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm, parvw_nm))
    rpt_parents_etl_sdf.registerTempTable(parvw_nm)
    #rpt_parents_etl_sdf.cache().show()

    utils.writeToLog("rpt_parents_etl_sdf - {}".format(rpt_parents_etl_sdf))
    ...
    sql = """select intab_period_id as period_id,
                    ...
                    unique_audience 
            from {parvw_nm} 
            order by 6 desc limit 200 """ .format(parvw_nm=parvw_nm)
...
final_df.repartition(1).write.format("com.databricks.spark.csv").option("header", "true").option("timestampFormat", "yyyy-MM-dd HH:mm:ss").mode('overwrite').csv("s3://useast1-nlsn-mdl-w-tam-totalmediafusion-dev/us_top200_sites_parents_20181008")
...
if __name__ == "__main__":
    ...
    main(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm, parvw_nm)
    # stop the SparkSession object
    spark.stop()
    utils.writeToLog("NOL US Top200 Sites Parents Application Completed")
