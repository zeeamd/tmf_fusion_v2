...
from extracts.src.main.commonsql.nol_adaptor_data_pull import get_report_credit_sql,get_nol_us_200channels

# Initiate BD3 spark Log
from library.src.main.commonutils import utils  #generic utilities

spark = utils.spark_session("Pyspark NOL US200 Channels integration")
...
try:
    ...
    proj_factors_sdf_frmt = get_nol_us_200channels(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm)
    proj_factors_sdf = utils.build_dataframe(spark, lambda: get_nol_us_200channels(intab_prd_id, intab_prd_stdt, intab_prd_endt, sch_nm, tab_nm))
...
proj_factors_sdf.repartition(2).write.format("com.databricks.spark.csv").option("header", "true").option("timestampFormat", "yyyy-MM-dd HH:mm:ss").mode('overwrite').csv("s3://useast1-nlsn-mdl-w-tam-totalmediafusion-dev/us_200channels_201821")
...
if __name__ == "__main__":
    ...
    spark.stop()
    print("NOL US 200Channels Application Job Completed")
